...
const tiers: Tier[] = [
  {
    name: 'Starter Access',
    basePrice: 49.00,
    features: ['Full app access', 'Standard automation flows', 'Continuous support']
  },
  {
    name: 'Innovator Tier',
    basePrice: 99.00,
    features: ['Full app + dashboard', 'Quarterly AI forecasts', 'Dedicated local support']
  },
  {
    name: 'Console Tier',
    basePrice: 149.00,
    features: ['Full app access + major consoles', 'Custom automation builds', 'Priority fast-track onboarding']
  },
  {
    name: 'Full Takeover',
    basePrice: 497.00,
    features: ['Custom AI ecosystem build', 'Exclusive console permissions', 'Direct 1-on-1 mentorship']
  }
];
                ...
                <div key={tier.name} className="price-card" style={styles.priceCard}>
                  <div>
                    ...
                    <div style={{ margin: '15px 0' }}>
                      <span style={styles.originalPrice}>${tier.basePrice.toFixed(2)}/mo</span>
                      <span style={styles.discountPrice}>${discountedPrice}<small style={{ fontSize: '0.9rem' }}>/mo</small></span>
                    </div>
                  ...
                  <button className="cta-btn" style={styles.cardBtn} onClick={() => handleOpenCheckout(tier)}>
                    Claim Tier
                  </button>
                </div>
              );
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            ...
            <form onSubmit={handleCheckoutSubmit}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Promo Code</label>
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  />
                  <button type="button" onClick={handleApplyPromo} style={styles.promoBtn}>Apply</button>
                </div>
                {promoStatus && (
                  <span style={{ fontSize: '0.85rem', marginTop: '5px', display: 'block', color: promoStatus.success ? '#4ade80' : '#f87171' }}>
                    {promoStatus.msg}
                  </span>
                )}
              </div>
              ...
              <div style={styles.totalBox}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <strong style={{ color: '#00F2FE', fontSize: '1.3rem' }}>
                    ${(selectedTier.basePrice * (1 - discountRate)).toFixed(2)}/mo
                  </strong>
                </div>
              </div>
...
// Inline CSS for clean React deployment
const styles: { [key: string]: React.CSSProperties } = {
  ...
  heroTitle: {
    ...
    fontWeight: 900,
    marginBottom: '18px',
    textShadow: '0 0 20px rgba(0, 0, 0, 0.8), 0 0 10px rgba(0, 242, 254, 0.5)',
  },
  heroText: {
    fontSize: '1.25rem',
    color: '#cbd5e1',
